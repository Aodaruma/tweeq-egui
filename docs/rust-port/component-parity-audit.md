...
| `InputButton` | `Button` | 近似 | Tweeq配色、hover/focus、disabled/invalidを実装。icon、chevron、tooltip、blink、narrow、連結角の細部が不足 |
| `InputButtonToggle` | `ToggleButton` | 近似 | boolean更新と専用状態描画、disabled/invalidを実装。icon、連結角、transitionの細部が不足 |
| `InputCheckbox` | `Checkbox` | 近似（高） | 専用24px描画、click/swipe、真偽shortcut、disabled/invalidを実装。icon差し替え、連結角、multi-select表示が残る |
| `InputSwitch` | `Switch` | 近似（高） | pill描画、click/swipe、真偽shortcut、disabled/invalidを実装。Vueの伸びるhandle animationと連結属性が残る |
| `InputRadio` | `Radio` | 近似（高） | segmented indicator、drag選択、矢印navigation、disabled/invalidを実装。icon-only/縦responsive、tooltip/labelizerが残る |
...
| `InputSize` | `Size` | 近似（高） | chain icon、link/unlink、ゼロ除算保護、disabled/invalidを実装。外部比率変更と単一edit sessionを要検証 |
| `InputTime` | `Timecode` | 近似（高） | 単一SMPTE field、単位文字列parse、H/M/S/T/F、単位別drag、Q snap、時計overlay、range/disabled/invalidを実装。frames/SMPTE menu、式、drop-frameが残る |
| `InputColor` | `ColorInput` | 近似 | 現状はegui標準RGBA picker。相対drag、H/S/V/A/R/G/B shortcut、popup構成、HSV pad/slider、presets、alpha、multi-editが不足 |
| `InputCubicBezier` | `CubicBezier` | 近似（高） | compact curve + popup picker、control line/handle、Q snap、矢印、4数値代替、disabled/invalidを実装。presetと複合edit sessionの整合が残る |
| `InputShuffle` | `Shuffle` | 近似 | Rustはseed付き数値range専用。Vueのgeneric `generate(prev)`、icon/dice animation、任意型の変更eventとはAPIが異なる |
...
| `Timeline` | `Timeline` | 近似 | 1本のscrubberのみ。tracks/keyframes/selection/zoom/scroll/edit modelが不足 |
...
2. ColorのHSVA pad/channel drag/presetを専用実装し、共通foreground primitiveへ寄せる。
